...
use app\common\model\Dingdan as Ddan;
...
class Dingdan extends Base
{
    ...
    {
        $result = Ddan::where('status',"neq",0)->paginate(5);
        foreach($result as $value)
        {
            $value->buserId = Buser::where('Id',$value->buserId)->value('bname');
            $value->suserId = Suser::where('Id',$value->suserId)->value('name');
            $foodId = explode(",",$value->food);
            for($i=0;$i<count($foodId);$i++)
            {
                $foodId[$i] = Foodlist::where('Id',$foodId[$i])->value('foodname');
            }
            $value->food = implode(",",$foodId);
        }
        $page = $result->render();
